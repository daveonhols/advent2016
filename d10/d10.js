...
function process(bot) {
    ...
    const lower = (bots[bot].values[0] > bots[bot].values[1]) ? bots[bot].values.pop() : bots[bot].values.shift();
...
function parseRule(parts) {
    ...
    return { who: who, low: { type: lowType, target: lowTarget }, high: { type:highType, target: highTarget } };
...
function run(input) {
    ...
    instructions.map( rule => { if (!rule.init) { setRule( rule.who, rule.low, rule.high ) } } );
    instructions.map( rule => { if (rule.init) { giveBot( rule.to, rule.val ) } } );

    const partTwo = [0, 1, 2].reduce( (sum, idx) => { return sum * outputs[idx]; }, 1 );
